...
- 长距离默认速度及第一轮 Task2 Build 后左移：1000 mm/s、800 ms 加速；两轮 Task2 的 2350 mm 和 2100 mm 坡道前进单独使用 800 mm/s、800 ms 加速。
...
最近一次部署记录：2026-09-20 已向树莓派同步长距离 1000 mm/s 和 Task2 两段坡道 800 mm/s、800 ms 加速配置，远端离线检查通过，现场效果待复测。此前已同步状态 6 支持、底盘并行和 Task1 舱门等待。现场日志证实 A 板已发出状态 6，但不能据此确认所有固件参数或整套动作均已实测。仓库与树莓派仍有既有橙色搜索代码差异，GitHub 上传不等于树莓派整库同步，见[部署记录](RaspberryPi/CHANGELOG.md)。
