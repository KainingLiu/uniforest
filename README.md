...
2026-08-25 调试收尾：完成三种方块的视觉目标整理，并将可现场修改的目标坐标、粗对准范围和末端微调范围集中到 `RaspberryPi/Strategy/vision_targets.py`。Task1 橙色目标为 `-1.0 mm`，Task2 紫色目标为 `-0.5 mm`，Task2 橙色目标选择最接近视觉中心的候选 `-0.1 mm`。本轮仅修改上位机策略配置、视觉目标入口和测试文档，UART7 协议、载荷格式、遥测布局及 A 板固件均未改变。
