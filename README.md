...
2026-09-04 上位机建筑对准与视觉标定：cube 相机 Linux 固定手动曝光（`exposure=312`、`gain=32`）并保留自动白平衡，避免运动中自动曝光引起轮廓顶边/颜色抖动；建筑对准新增并切换到包含亮顶面的 `building` 检测色带，纵向收敛目标保持 `X=0 mm、Z=75 mm`；对准改为“横向优先、每个轴仅在超出自身容差时动作、可重新纠正漂移”，并加入纵向减速带抑制窗口边缘振荡。Purple 色带按手动曝光+自动白平衡重新标定为 `H105-155`，恢复紫色识别。协议未变（UART7 命令编号、80 字节遥测布局与 A 板固件均未改变）。
...
Tag 对准已恢复为原有的距离、横向和航向同时闭环控制；本轮未改变通信协议。
Tag6 附近横向动作如下：

| 轮次 | Task1：Tag6 后右移 | Task1：最终右转前左移 | Task2：Tag6 后右移 |
| --- | ---: | ---: | ---: |
| 第一轮 | 100 mm | 100 mm | 100 mm |
| 第二轮 | 400 mm | 400 mm | 400 mm |

建筑视觉对准 + Build 的独立测试程序为 `RaspberryPi/building_build_test.py`，
仅执行建筑对准和 Build，不执行 Tag6 横移或航向转动。
