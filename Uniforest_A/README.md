...
2026-09-24 Grap3 收尾：同时垂直上升 9 cm、水平回收 5 cm，本段上升到 5 cm
时前臂复位 90°，两轴结束后全部舵机复位。无新增等待或底盘就绪节点，须烧录生效。

2026-09-24 吸气切换修复：新抓取在前一次释放后的 1 秒内开始时，`Suction_PumpOn()`
必须向 TIM4 CH2 写入关阀 PWM，再开启气泵。旧实现只清除关阀计时和软件状态，
可能让释放阀持续打开；本次补齐实际关阀输出，不增加动作等待。需重新烧录后验证。
用户已确认这是 Task2-R2 末次橙色抓取失败的原因；此反馈不代表已核实板上完整固件版本。
