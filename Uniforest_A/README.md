...
前臂更换后的固定零点补偿位于 `Core/Inc/servo.h` 的
`SERVO_ARM_FRONT_OFFSET_TENTH=120`（0.1° 单位，即 +12°；2026-09-23 用户确认
新舵机输出 102° 对应原逻辑 90°）。
`Servo_SetAngle()` 统一经 `Servo_SetAngleTenth()` 输出，对前臂 ID 1 加一次偏置，
覆盖开机复位、串口控制和 Grap/Build；其他舵机不加偏置。
逻辑默认角度仍为 90°，偏置须通过 CLion 烧录后生效；目前未确认已烧录或全动作复测。
操作方法与输出限幅见[前臂零点调整](../RaspberryPi/tools/arm_zero_adjust.md)。
...
当前 Grap1/2 的垂直下降及回程上升均为 18.5 cm，水平与 5 cm 就绪触发位置保持。该变更和前臂 +12° 偏置均需重新烧录；尚未收到最新固件烧录与现场复测确认。
