...
#define M3508_SPEED_OUT_LIMIT       15000.0f

/* Current loop is reserved; C620 closes the motor current loop internally. */
...
#define M3508_CURRENT_OUT_LIMIT     15000.0f
...
#define M3508_GEAR_RATIO              (3591.0f / 187.0f)   /* exact P19 gearbox */
#define WHEEL_DIAMETER_MM            152.4f
...
/**
 * @brief  Read motor feedback for telemetry (no locking)
 * @param  motor_id    M3508_IDX_TR (0) … M3508_IDX_BR (3)
 * @param  angle       [out] raw encoder angle (0–8191)
 * @param  speed_rpm   [out] rotational speed (RPM)
 * @param  torque      [out] actual torque current
 * @param  temperature [out] motor temperature (℃)
 */
...
/**
 * @brief  Read the multi-turn encoder position accumulated from CAN feedback.
 * @note   This value is not reset by an upper-computer move; the Pi records an
 *         action-local origin and subtracts it from subsequent telemetry.
 */
 ...
 * @param  rpm[4]  desired RPM for each motor
 */
 ...
 * @param  kp, ki, kd  proportional, integral, derivative gains
 * @param  integral_limit  anti-windup clamp
 * @param  output_limit    output clamp
 */
...
/**
 * @brief  Reset PID integrators and error history for a motor
 */
void Motor3508_ResetPID(uint8_t motor_id);

/* ---------- Distance Move API (Legacy — Pi-controlled now) ---------- */
...
void Motor3508_Turn(float target_deg, float speed_deg_s);
                /* +target_deg = CW (右转), -target_deg = CCW (左转) */
