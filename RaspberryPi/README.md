...
2026-09-23 已定向上传当前 Task2 路线、Tag6 容差 ±10 mm/取消精对准、
最低平移速度 80 mm/s、常规路线转向前馈 120°/s，以及此前方块对准提速和心跳日志调整。
远端无硬件检查通过；2026-09-24 整理文档，待树莓派连接后同步，不自动启动实机任务。

| 项目 | 当前源码 / 已知部署状态 |
| --- | --- |
| 上位机路线与对准参数 | 最新参数已定向上传，两轮配置已核对；现场效果待复测 |
...
| Task1 标签对准 | 前后最高 350，左右最高 280；容差外非零期望最低 50 mm/s | 300 mm/s² |
...
2. 两轮均直接以 400 mm/s 前进 300 mm，再以 300 mm/s 向前顶墙。
...
8. 转至 180° 航向，以 800 mm/s、800 ms 加速前进 2800 mm，对准 Tag6 至 425 mm、横向 0 mm；距离与横向容差均为 ±10 mm，航向容差 ±3°。取消精对准，三项合格后停车，连续确认 4 个新帧即继续。
...
Tag6 对准和相机预检仍保留。Task2 两轮 Tag6 使用 ±10 mm 距离/横向容差、取消精对准；Task1 的 Tag6 参数保持原设置。
Task2 标签对准在容差外的非零平移期望速度最低为 80 mm/s，前后和横向共用；
