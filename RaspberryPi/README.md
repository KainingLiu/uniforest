...
| 项目 | 当前状态 |
| --- | --- |
| Tag6 两轴决定完成、持续航向纠偏 | 09-26 已定向同步两任务两轮；保留四帧确认，远端检查通过，现场效果待确认 |
...
- 比赛定距路线：Task0 和两轮 Task1/Task2 的 `_checked_move()` 显式启用
...
- 手动/Agent 单次定位默认 `route_mode=False`，保留 1000 counts（约 3 mm）窗口、
...
6. 对准 Tag6 至距离 425 mm、横向零点；前后/横向单轴进入 ±8 mm 即停止该轴平移，两帧超差后重启。航向持续向 180° 修正，不参与完成判定；两平移轴合格连续确认 4 个新帧后停车继续，不执行精对准。按轮次向右定距移动后顶墙卸载。
...
8. 转至 180° 航向，以 800 mm/s、800 ms 加速前进 2800 mm，对准 Tag6 至 425 mm、横向 0 mm；距离与横向容差均为 ±8 mm。航向持续向 180° 修正，不设置角度容差或死区，不参与完成判定；两平移轴连续合格 4 个新帧后停车继续。取消精对准。
...
50 ms，仅用有效新帧更新；航向由陀螺仪持续纠偏。两任务均取消精对准。

| 参数 | Task1 两轮 | Task2 两轮 |
| --- | ---: | ---: |
| 距离目标 / 容差 | 425 / ±8 mm | 425 / ±8 mm |
| 横向目标 / 容差 | 0 / ±8 mm | 0 / ±8 mm |
| 航向目标 / 完成条件 | 180° / 不参与完成判定 | 180° / 不参与完成判定 |
...
前后、横向单轴进入容差后立即停止该轴平移并清零其 PID 和速度状态，跳过软件
减速；该轴连续两个有效新帧超差才重新启动。航向 PID 持续向 180° 修正，忽略
原 ±3° 容差、0.5° 死区和航向停车保持；只在误差恰为 0 时停止旋转。两平移轴
同时合格连续四个新帧即完成，确认期间继续修正航向，完成后四轮零速。
