...
Build 前，使用 cube 摄像头从斜上方识别三层橙色建筑整体轮廓；通过轮廓高宽比
`0.45-1.30` 适应完整路线后观察角度造成的轮廓变化，并排除明显扁平的橙色轮廓；以当前连续 8 秒复测的中位位置
`X=-8.4 mm、Z=150.0 mm` 作为当前二维平移 PID 收敛目标。连续 5 个新帧进入横向
`±3 mm`、前向 `±6 mm`、航向 `±2.4°` 的允许范围后执行 Build。目标、容差、
PID、限速和超时均集中在 `Task2Config` 的 `building_*` 参数中，供后续精调。
建筑平移 PID 的最低线速度为 100 mm/s，以克服小位移横移时的底盘静摩擦；前后和横向比例增益
分别为 1.5 和 1.8。
