...
启用 Robot 方块视觉后，仅在抓取前的橙色/紫色搜索和对准阶段自动采集原图，
...
| Agent 与视觉目录拆分 | 本地已合并 GitHub 的 590dca2；本次部署不覆盖远端 Agent，树莓派仍保留原 `vision/` 目录及标定位置 |
| Robot 兼容接口 | 两端均有 `quiet_heartbeat` 与采集接口；远端其他既有差异保留 |
...
速度可以低于 100 mm/s；合格轴允许零速。视觉丢失超时或总超时仍报错中止，不按
常规路线转向的超时降级规则放行。

09-26 最低平移速度调整为 100 mm/s，部署和验证记录见 CHANGELOG。当前关闭的 Tag3 共用滤波、增益和最低速度，但不启用
