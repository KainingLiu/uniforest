...
压墙完成；堵转成立或达到 1 秒后给底盘零速，机械臂继续抓取，两者均结束才继续流程。
橙色短压墙结束时仍重校准航向；紫色沿用不重校准的设置。动作拒绝、取消、通信或遥测
异常会中止组合动作。常规顶墙 250 mm/s 及并行抓取的效果待现场验证，协议未变。

常规顶墙为 250 mm/s、最长 4 秒；与抓取同时启动的短压墙为 150 mm/s、最长 1 秒。前向顶墙检查两个后轮，侧向顶墙使用四轮中至少三轮，低速/高电流阈值及确认时间在 `FirstTaskConfig`。
...
1. 以 250 mm/s 向前顶墙并重新标定当前航向零点。
2. 以 300 mm/s 连续向右搜索橙色，累计搜索上限 1800 mm；锁定目标后进行粗对准和末端微调。
3. 对准后同时启动 Grap3 和 150 mm/s 短压墙，短压墙结束后重新校准航向；完成 3 个方块。
...
2. 第一轮以 400 mm/s 右移 100 mm，第二轮跳过；两轮均以 400 mm/s 前进 250 mm，再以 250 mm/s 向前顶墙。
...
5. 两轮均先以 250 mm/s 向左顶墙，再向前顶墙，重新校准航向。
...
10. 第一轮 Build 后以 400 mm/s 后退 100 mm，顺时针相对转 180°，以 750 mm/s 左移 2500 mm，最后以 250 mm/s 左顶墙结束。第二轮在 Build 完成后直接结束。
