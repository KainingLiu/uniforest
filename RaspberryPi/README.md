...
Build 前使用 cube 摄像头识别三层橙色建筑，并以轮廓上边缘作为 X/Z 对准参照（避免下半部分遮挡造成偏移）。对准期间自动切换到 `building` 检测色带（Orange H0-50、S≥35），把亮顶面并入同一橙色轮廓；默认 Orange 带会漏掉亮顶面，使顶边落在前脸交界、Z 偏低并诱导多余后退。cube 相机当前固定手动曝光/增益并保留自动白平衡（见 `vision/camera_settings.json`）；若现场光照或曝光改变导致顶边行偏移，用 `tools/building_vision_probe.py --profile building` 复测，并按 `75 × 顶边行` 重设 `Task2Config.building_z_scale_mm_px`。收敛目标 `X=0.0 mm、Z=75.0 mm`。轮廓高宽比门限 `0.35-2.20`，置信度 ≥35%，视觉帧保留 0.7 s，目标丢失保持 4 s；目标先经 2 个连续帧锁定，锁定后只接受横向跳变 ≤50 mm、前后跳变 ≤60 mm 的同一轮廓，并用中位滤波位置作为下一次门限基准，抑制顶边单帧跳变。对准采用“横向优先”：每个轴只在超出自身容差（横向 `±3 mm`、前向 `±6 mm`、航向 `±2.4°`）时才动作，已对齐的轴不再以最低速度反复踢动，漂移出容差可重新纠正。远处前后逼近以 100 mm/s 最低速克服静摩擦；剩余前后误差 <30 mm 时进入 60-90 mm/s 减速带，避免在容差窗口边缘反复冲过头。连续 3 个新帧同时入容差后执行 Build。参数集中于 `Task2Config` 的 `building_*`。若建筑目标丢失或视觉对准超时，程序先停车并输出警告，再继续执行 Build；底盘通信等其他异常仍按故障处理。
...
```bash
.venv/bin/python tools/building_vision_probe.py --camera cube --profile building --duration 5
```
