...
截至 2026-09-20，数量检查、机构并行、舱门等待及最新速度配置均采用定向部署，未整库覆盖树莓派。GitHub 以当前源码
...
最近一次同步包含长距离默认 1000 mm/s，以及两轮 Task2 两段坡道的独立
800 mm/s、800 ms 加速配置。远端离线检查通过，未运行实机动作；部署备份及
验证结果见 [CHANGELOG.md](CHANGELOG.md)。2026-09-21 的文档整理与 GitHub 上传不另行部署树莓派。
...
| 两轮 Task2 起步 2350 mm、前往 Build 2100 mm（坡道） | 800 mm/s | 800 ms |
...
1. 以 800 mm/s 前进 2350 mm，转到 -90° 航向，对准 Tag3 至 250 mm。
2. 第一轮以 400 mm/s 右移 100 mm，第二轮跳过；两轮均以 400 mm/s 前进 250 mm，再以 300 mm/s 向前顶墙。
...
8. 转至 180° 航向，以 800 mm/s 前进 2100 mm，对准 Tag6 至 425 mm。
...
- 距目标 180 mm 及以上时期望速度 420 mm/s，30–180 mm 间随距离收缩；
  30 mm 内、尚未进入合格窗口时期望速度为 100 mm/s。最终期望上限为 420 mm/s。
- 从静止起步指令为 40 mm/s，速度变化限制为 300 mm/s²；反向前先发零速度。
  进入合格范围时直接发零速度，不经缓慢减速，也不发送整车急停命令。
- 合格窗口内累计 3 个新帧样本后通过；已进入窗口后若只越界 5 mm 以内，
  最多停车保持 200 ms。边缘保持不增加确认次数，但保留已有次数。
- 橙色粗对准后执行最长 1 s 的 ±3 mm 微调，使用同一速度规则。微调超时、
  本阶段有有效样本且最后记录位置仍在粗窗口内时允许抓取；不保证每次达到 ±3 mm。
- 新画面中目标无效时先停车，持续丢失 0.5 s 返回搜索；普通对准 10 s 超时报错。
  对准成功后，Grap3（Task1 橙色）、Grap1（Task2 橙色）或 Grap2（紫色）
  与 150 mm/s 短压墙同时启动。

2026-09-21 源码核对发现、尚未修复的边界：重复时间戳分支会跳过图像过期及
丢失检查，因此画面冻结不保证 0.5 s 停车，可能保留上一速度直到对准总超时；
短暂丢失虽发送零速度，但未清零内部上一横移速度，重新识别后可能沿用停车前的
速度状态。以上是源码分析，未进行实机复现；本次文档整理不改变控制行为。
