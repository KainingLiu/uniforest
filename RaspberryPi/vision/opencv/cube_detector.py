...
class CubeDetector:
    ...
    def start(self) -> bool:
        ...
        self._camera_metadata = {
            "selector": self._camera_id,
            "source": str(self._camera_source),
            "requested": {"width": self._resolution[0],
                          "height": self._resolution[1],
                          "exposure": self._exposure, "gain": self._gain,
                          "white_balance": self._white_balance},
            "reported": {
                "width": actual_w, "height": actual_h,
                "fps": read_property(cv2.CAP_PROP_FPS),
                "exposure": read_property(cv2.CAP_PROP_EXPOSURE),
                "gain": read_property(cv2.CAP_PROP_GAIN),
                "auto_white_balance": read_property(cv2.CAP_PROP_AUTO_WB),
                "white_balance": read_property(cv2.CAP_PROP_WB_TEMPERATURE),
            },
            "calibration": {key: self._state[key]
                            for key in ("fx", "fy", "cx", "cy", "calibrated")},
        }
    ...
    def set_frame_sink(self, sink):
        """Attach an optional non-blocking raw-frame consumer; no extra camera."""
    ...
    def _capture_loop(self):
        ...
        while self._running:
            ...
            with self._result_lock:
                self._raw_frame = (frame.copy(), captured_monotonic)
