...
2026-09-26：自动采集仅接收抓取前橙色、紫色搜索和对准阶段的未标注图片。数据保存在运行程序的
...
`robot.py --vision` 和启用视觉的 Agent Robot 会使用同一阶段过滤机制；仅打开视觉、
没有进入比赛搜索/对准阶段时不保存图片。Agent 执行已有比赛任务时随任务阶段采集。
未启用视觉的纯底盘/通信调试不会为了采集自动启动相机。

| 自动采集阶段 | 允许的视觉 profile |
| --- | --- |
...
一个 session，可能产生无图片批次。允许阶段已入队的图片会正常保存，保留原阶段信息。
...
`capture` 是显式手动采集命令，不受比赛阶段过滤；它仍遵守采样、存储和队列限制。
...
在允许采集的阶段内，任务阶段、flow 或 profile 改变时，尝试保留下一张新帧，跳过
常规抽样间隔和去重；补抓返回同一搜索阶段也保留首帧。队列和存储限制仍然生效。
采集不以传统检测器是否找到目标作为条件，
因此漏检、空场和遮挡画面都有机会保留。
